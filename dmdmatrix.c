...
void spi_init (void);		//spi initializaton
...
void initMatrix() 
{
	...
	for(i = 0; i < NUM_DEVICES; i++)
	{
		...
		writeByte(0x01); // Disable Display-Test
	}
	LPC_GPIO0->FIOSET = 1<<23;	//release slave
}
...
LPC_PINCON->PINSEL1 |= 0x0000003F;	//Pin P0.18 allocated to function 4 MOSI
LPC_GPIO0->FIODIR |= (1<<15)| (1<<18)| (1<<23);	  //p0.15, P0.18. P0.23 are outputs
LPC_SC->PCONP |= 1 <<8;	//enable POWER to SPI (redundant following reset)
LPC_SC->PCLKSEL0 |= 1<<16;	//pclk = cclk
LPC_SPI->SPCCR |= (0x00000050);	//clock divider
LPC_SPI->SPCR |= 0x00000026;
